...
export default function AssessmentClient({ tenantSlug }: { tenantSlug: string }) {
  ...
  if (!tenantSlugValue) {
    return (
      <div className="mx-auto flex min-h-[40vh] w-full max-w-3xl items-center justify-center px-6 py-8">
        <div className="rounded-2xl border border-red-500/20 bg-red-500/10 p-6 text-sm text-red-200">
          Tenant inválido o ausente.
        </div>
      </div>
    );
  }
  ...
  const handleDownloadPdf = () => {
    ...
    const premiumDiagnosis = aiAnalysis?.executiveDiagnosis || executiveDiagnosisText;
    const premiumImplication = aiAnalysis?.strategicImplication || strategicImplicationText;
    ...
    metricCard(marginX, y, cardWidth, "Estado", scoreLabel, "Lectura ejecutiva");
    metricCard(
    ...
    bulletList(premiumOpportunities.slice(0, 5), "No se identificaron oportunidades destacadas.");
    ...
    bulletList(premiumActions, "No se definieron acciones recomendadas.");
            ...
            <div className="space-y-5">
              ...
              <div className="space-y-4">
                {currentBlock.questions.map((question) => {
                  const answer = currentAnswer.answers.find(
                ...
                <button
                  ...
                  onClick={() => {
                    ...
                    setCurrentIndex((prev) =>
                      Math.min(prev + 1, diagnosticBlocks.length - 1),
                    );
                  }}
                ...
                onAction={async (action: { type: string; documentId: string }) => {
                  ...
                  if (action.type === "open_upload") {
                    ...
                    try {
                      ...
                      formData.append("description", `Carga simulada para ${targetDoc.title}`);
                      ...
                      if (data.ok) {
                        const mergedDocuments = mockDocuments.map((mockDoc: TenantDocument) => {
                          ...
                          return storedDoc ? { ...mockDoc, ...storedDoc } : mockDoc;
                        });
            ...
            <div className="rounded-2xl border border-white/10 bg-white/[0.04] p-5">
              ...
              {analysisMode === "premium" && aiAnalysis ? (
                <div className="mt-4 rounded-xl border border-[#C96F3B]/20 bg-[#0F2422] p-4">
                  ...
                  {aiAnalysis.summary && (
                    <div className="mt-2 text-sm text-white/90">{aiAnalysis.summary}</div>
                  )}
                  ...
                  {Array.isArray(aiAnalysis.opportunities) &&
                    aiAnalysis.opportunities.length > 0 && (
                      <div className="mt-3">
                        ...
                        <ul className="mt-1 space-y-1 text-sm text-white/80">
                          {aiAnalysis.opportunities.slice(0, 3).map((o: string, i: number) => (
                            <li key={i}>• {o}</li>
                          ))}
                        </ul>
                      </div>
              ...
              {analysisMode === "premium" && aiAnalysis ? (
                <>
                  ...
                  <div className="mt-4 rounded-xl border border-white/10 bg-white/[0.03] p-4">
                    ...
                    <div className="mt-1 text-xs text-white/60">Próximos pasos sugeridos</div>
              ...
              {analysisMode === "premium" && aiAnalysis?.priority ? (
                <div className="mt-4 rounded-xl border border-[#C96F3B]/30 bg-[#C96F3B]/10 p-4">
                  ...
                  <div className="mt-2 text-sm text-white">{aiAnalysis.priority}</div>
              ...
              <div className="mt-4 rounded-xl border border-red-500/20 bg-[#0F2422] p-4">
                ...
                <div className="mt-3">
                  {Array.isArray(weakestBlocks) && weakestBlocks.length > 0 ? (
                    <div className="text-sm text-white">{weakestBlocks[0].title}</div>
                  ) : (
                ...
                {analysisMode === "premium" && aiAnalysis?.priority ? (
                  <div className="mt-3 rounded-lg border border-[#C96F3B]/20 bg-[#C96F3B]/10 p-3">
                    ...
                    <div className="mt-1 text-sm text-white/85">{aiAnalysis.priority}</div>
                  </div>
                ) : null}
              </div>
              ...
              <div className="mt-4 space-y-3">
                {quickViewBlocks.map((block) => {
                  ...
                  const scoreLabel = block.isDocumentBlock
                    ? "Docs"
                    : (() => {
                        const diagnosticBlock = diagnosticBlocks.find(
                          (b) => b.id === block.id,
                        );
                        ...
                        {!block.isDocumentBlock ? (
                          <div className="mt-2 h-1.5 w-full overflow-hidden rounded-full bg-white/10">
                            <div
                              ...
                              style={{
                                width: `${
                                  blockScores.find((item) => item.blockId === block.id)
                                    ?.percentage ?? 0
                                }%`,
                              }}
                            />
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/70 px-4">
          <div className="w-full max-w-2xl rounded-2xl border border-white/10 bg-[#0F2422] p-6 shadow-2xl">
            <div className="flex items-start justify-between gap-4">
              <div>
                <p className="text-xs uppercase tracking-[0.24em] text-[#C96F3B]">Foco IA</p>
            ...
            <div className="mt-4 rounded-xl border border-white/10 bg-black/20 p-4 text-sm leading-6 text-white/85 whitespace-pre-wrap">
              {focusHelpLoading ? "Generando explicación..." : focusHelpText || "Sin contenido."}
            </div>
          </div>
        </div>
      ) : null}
