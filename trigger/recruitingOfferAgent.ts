...
export const recruitingOfferAgentTask = task({
  id: "recruiting-agent-offer",
  ...
  run: async (payload: AgentPayload) => {
    ...
    try {
      const result = await runRecruitingAgent({ ...payload, agentType: "offer", taskType: "trigger.offer" });
      if (payload.taskId) {
        await prisma.recruitingAgentTask.update({
          where: { id: payload.taskId },
      ...
      logger.info("Offer agent completed", { candidateId: payload.candidateId });
