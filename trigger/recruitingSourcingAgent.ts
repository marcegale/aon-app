...
export const recruitingSourcingAgentTask = task({
  id: "recruiting-agent-sourcing",
  ...
  run: async (payload: AgentPayload) => {
    ...
    try {
      const result = await runRecruitingAgent({ ...payload, agentType: "sourcing", taskType: "trigger.sourcing" });
      if (payload.taskId) {
        await prisma.recruitingAgentTask.update({
          where: { id: payload.taskId },
      ...
      logger.info("Sourcing agent completed", { searchId: payload.searchId });
