...
function formatAmount(value: number | string | undefined) {
  ...
  return new Intl.NumberFormat("de-DE", { maximumFractionDigits: 0 }).format(numberValue);
}
  ...
  if (loading) {
    return (
      <div className="space-y-5">
        <Link href={HISTORY_BASE} className="text-sm text-[#C96F3B] hover:text-[#F4EBD0]">
          ← Volver al histórico
        </Link>
        <div className="rounded-[28px] border border-white/10 bg-[#0F2422] p-8 text-white/50">
          Cargando lote...
        </div>
      </div>
    );
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid gap-4 md:grid-cols-4">
        <div className="rounded-2xl border border-white/10 bg-[#0F2422] p-5">
        ...
        <div className="rounded-2xl border border-[#C96F3B]/20 bg-[#C96F3B]/10 p-5">
          <p className="text-xs uppercase tracking-[0.16em] text-[#F4EBD0]/70">Estado</p>
          <p className="mt-2 text-lg font-semibold capitalize text-[#F4EBD0]">{status}</p>
        </div>
      </div>
      ...
      <div className="overflow-hidden rounded-[24px] border border-white/10 bg-[#0F2422]">
        <div className="flex flex-col gap-3 border-b border-white/10 p-6 sm:flex-row sm:items-center sm:justify-between">
          ...
          <button
            ...
            className="rounded-xl bg-[#C96F3B] px-5 py-3 text-sm font-semibold text-white transition hover:bg-[#B85E30] active:scale-[0.99] disabled:cursor-not-allowed disabled:bg-white/10 disabled:text-white/30"
          >
            Reexportar lote
          </button>
        </div>
        ...
        {batchItems.length > 0 ? (
          <div className="overflow-x-auto">
            ...
            {batchItems.map((item, index) => {
              const parsed = item.parsed;
              const provider =
                parsed?.proveedor || parsed?.razonSocialEmisor || "Proveedor no identificado";
          ...
          <div className="p-8 text-sm leading-6 text-white/45">
            Los lotes antiguos pueden no tener detalle por factura. Procesá y exportá un lote
            nuevo para ver la tabla completa.
          </div>
        )}
      </div>
    </div>
