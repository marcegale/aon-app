...
class UIBridge:
    ...
    OrbWindow and CockpitWindow never reference the broker directly.
    ...
    def _on_state_changed(self, payload: Any) -> None:
        if not isinstance(payload, dict):
            return
        fsm_state = payload.get("to")
        if not fsm_state:
            return

        orb_state = fsm_to_orb(fsm_state)
        logging.info("[ui_bridge] %s → %s", fsm_state, orb_state)
